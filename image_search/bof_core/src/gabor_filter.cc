...
/*!
  \fn GaborFilter::GaborFilter(int iMu, int iNu, double dSigma)
  Construct a gabor
  ...
  Create a gabor with a orientation iMu*PI/8, a scale iNu,
  and a sigma value dSigma. The spatial frequence (F) is set to sqrt(2) defaultly.
...
/*!
  \fn GaborFilter::GaborFilter(int iMu, int iNu, double dSigma, double dF)
  Construct a gabor
...
/*!
  \fn GaborFilter::GaborFilter(double dPhi, int iNu)
  Construct a gabor
...
/*!
  \fn GaborFilter::GaborFilter(double dPhi, int iNu, double dSigma, double dF)
  Construct a gabor
...
/*!
  \fn GaborFilter::get_image(int Type)
...
*/
IplImage* GaborFilter::get_image(int Type)
{
...
*/
long GaborFilter::get_mask_width()
{
  return Width;
}

/*!
  \fn GaborFilter::Init(int iMu, int iNu, double dSigma, double dF)
  Initilize the.gabor
  ...
  Initilize the.gabor with the orientation iMu, the scale iNu, the sigma dSigma,
...
/*!
  \fn GaborFilter::Init(double dPhi, int iNu, double dSigma, double dF)
  Initilize the.gabor
  ...
  the frequency dF, it will call the function creat_kernel();
  So a gabor is created.filename    The name of the image file
...
/*!
  \fn GaborFilter::get_matrix(int Type)
...
*/
CvMat* GaborFilter::get_matrix(int Type)
{
...
/*!
  \fn GaborFilter::output_file(const char *filename, Gan_ImageFileFormat file_format, int Type)
...
*/
void GaborFilter::output_file(const char *filename, int Type)
{
  IplImage *pImage;
...
/*!
  \fn GaborFilter::show(int Type)
*/
void GaborFilter::show(int Type)
{
  if(!IsInit()) {
...
/*!
  \fn GaborFilter::conv_img_a(IplImage *src, IplImage *dst, int Type)
*/
void GaborFilter::conv_img_a(IplImage *src, IplImage *dst, int Type)
{
  double ve, re,im;
...
/*!
  \fn GaborFilter::GaborFilter(int iMu, int iNu)
*/
...
/*!
  \fn GaborFilter::normalize( const CvArr* src, CvArr* dst, double a,
                              double b, int norm_type, const CvArr* mask )
*/
void GaborFilter::normalize( const CvArr* src, CvArr* dst, double a,
                             double b, int norm_type, const CvArr* mask )
{
  CvMat* tmp = 0;
  /*  __BEGIN__;*/
...
/*!
  \fn GaborFilter::conv_img(IplImage *src, IplImage *dst, int Type)
*/
void GaborFilter::conv_img(IplImage *src, IplImage *dst, int Type)
{
  double ve;
  CvMat *mat = cvCreateMat(src->width, src->height, CV_32FC1);
