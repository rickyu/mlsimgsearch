...
CLogger& g_logger = CLogger::GetInstance("searcher");
...
int main(int argc, char **argv) {
  ...
  BEGIN_SEARCH_SERVICE_MAP()
    REGISTER_SERVICE(CommonSearch)
  END_SEARCH_SERVICE_MAP;
  ...
  framework.Init();
  boost::shared_ptr<ImageSearchMsgHandler> handler(new ImageSearchMsgHandler());
  ret = handler->Init();
  if (CR_SUCCESS != ret) {
    LOGGER_ERROR(g_logger, "act=ImgGenMsgHandler failed, ret=%d", ret);
    return -4;
  }
  ImageSearchMsgProcessor processor(handler);
