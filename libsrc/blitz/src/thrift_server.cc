  ...
  void operator()() {
    if (expire_time_ > 0) {
      ...
      if (cur_time > expire_time_) {
        ...
        LOGGER_ERROR(g_log_framework, "ThriftSyncServer tasktimeout");
        return;
      }
    }
    server_->RealProcessMsg(ioinfo_, msg_);
  }
...
int ThriftSyncServer::ProcessMsg(const IoInfo& ioinfo,const InMsg& msg) {
  ...
  MsgHandler handler(this, ioinfo, msg, expire_time);
...
int ThriftSyncServer::RealProcessMsg(const IoInfo& ioinfo, const InMsg& msg ) {
  static CLogger& g_log_framework = CLogger::GetInstance("framework");
