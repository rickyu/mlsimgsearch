#include "blitz/memcached_server_msg.h"
namespace blitz {
const int MemcachedServerMsgDecoder::kDefaultMaxMemcachedServerLength = 150;

int MemcachedServerMsgDecoder::GetBuffer(void** buf,uint32_t* length) {
...
int MemcachedServerMsgDecoder::Feed(void* buf, uint32_t length, std::vector<InMsg>* msgs) {
...
int MemcachedServerMsgDecoder::PrepareBuffer() {
  if (!buffer_) {
