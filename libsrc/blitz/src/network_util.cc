...
int NetworkUtil::GetLocalName(int fd, char* buf, size_t buf_length) {
  ...
  int ret = getsockname(fd, reinterpret_cast<sockaddr*>(&addr),
      &length);
  if (ret == 0) {
    if (GetAddrString(addr, buf, buf_length) ) {
      return 0;
    }
  }
  return -1;
...
const char* NetworkUtil::GetAddrString(const sockaddr_in& addr, char* buf,
                                      size_t buf_length) {
  char ip_buf[16];
  ...
  snprintf(buf, buf_length, "%s:%d", str, ntohs(addr.sin_port));
  return buf;
