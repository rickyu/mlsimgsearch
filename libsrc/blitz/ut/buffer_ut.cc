...
TEST(BufferWriter, Init) {
  ...
  ret = w.Init(1024);
  EXPECT_EQ(ret, blitz::BufferWriter::kAlreadyInited);
  const blitz::DataBuffer* buffers = w.get_buffers();
...
TEST(BufferReader, constructor) {
  blitz::DataBuffer data;
  blitz::DataBufferUtil::Zero(&data);
  data.data_length = 10;
  blitz::BufferReader r(&data, 1);
...
TEST(BufferReader, ReadByte) {
  ...
  ret = r.ReadByte(&byte);
  EXPECT_EQ(ret, blitz::BufferReader::kNotEnoughData);
  EXPECT_EQ(r.get_data_length(), 3);
  EXPECT_EQ(r.get_data_left_length(), 0);
}
